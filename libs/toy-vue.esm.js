...
const displayString$1 = 'displayString';
const DISPLAY_STRING = Symbol(displayString$1);
const CREATE_ELEMENT = Symbol(createElementVNode);
const helperMapName = {
    [DISPLAY_STRING]: displayString$1,
...
function transformText(node) {
    if (node.type === 2 /* NodeTypes.ELEMENT */) {
        return () => {
            const { children } = node;
            const isTextOrInterpolation = (node) => node.type === 3 /* NodeTypes.TEXT */ || node.type === 0 /* NodeTypes.INTERPOLATION */;
...
function genFunctionPre(ast, context) {
    const alias = (val) => `${helperMapName[val]}: _${helperMapName[val]}`;
...
export { ReactiveEffect, computed, convertToReactive, createApp, createVNode as createElementVNode, createRender, createTextVNode, createVNode, displayString, effect, getCurrentInstance, h, inject, isProxy, isReactive, isReadonly, isRef, isTracking, nextTick, provide, proxyRefs, reactive, readonly, ref, registerRuntimeCompiler, renderSlots, renderer, shallowReadonly, stop, track, trackEffects, trigger, triggerEffects, unRef };
